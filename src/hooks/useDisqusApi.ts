...
export const useDisqusThread = (shortname: string, identifier: string | undefined, apiKeys: string[], api = 'https://disqus.com/api/') => {
  /*
   * 获取 Thread 信息
   ...
   */
  return useFetch<DisqusAPI.Thread>(typeof identifier === 'string' ? `${api}3.0/threads/list.json?forum=${encodeURIComponent(shortname)}&thread=${encodeURIComponent(`ident:${identifier}`)}` : null, disqusJsApiFetcher(apiKeys));
};
...
export const useDisqusPosts = (shortname: string, id: string | null, apiKeys: string[], api = 'https://disqus.com/api/') => {
   ...
   */
  return useFetchInfinite<DisqusAPI.Posts>(
    getKey,
